...
const ContactItemDialog = ({ isOpen, onClose, contact, rating }: FriendAnalyticDialogProps) => {
	return (
			...
			<DialogContent className='p-1'>
				<Tabs defaultValue='statistic' className='border-0 shadow-none'>
					<TabsList className='gap-3 '>
						...
						<TabsTrigger value='edit'>Профиль</TabsTrigger>
					</TabsList>
					<TabsContent value='statistic' className='border-0'>
						<DialogHeader className='flex items-end gap-3 flex-row'>
							...
							<div>
								<DialogTitle className='text-lg font-semibold'>{contact?.name}</DialogTitle>
							</div>
						</DialogHeader>
